...
impl FungibleTokenReceiver for Contract {
    #[payable]
    fn ft_on_transfer(&mut self, sender_id: ValidAccountId, amount: U128, msg: String) -> U128 {
        // artificially inflate storage req
        let min_balance_to_contract = u128::from(self.storage_minimum_balance()) * 2;
        // get balance for original tx sender of tokens to a token receiving contract
        let storage_balance = u128::from(self.storage_balance_of(sender_id).total);

        env::log(
            format!(
                "attached {}, min_balance_to_contract {}, storage_balance {}, amount {:?}, msg {}",
                env::attached_deposit(), min_balance_to_contract, storage_balance, amount, msg
            )
            .as_bytes(),
        );

        if storage_balance < min_balance_to_contract {
            // try to register more storage with attached deposit
            if env::attached_deposit() < min_balance_to_contract - storage_balance {
                // can't register storage should panic
                env::panic("not enough storage to have token contract store balance".as_bytes())
            }
            // will need to modify storage_manager to accept sender_id, e.g. modified storage for sender not token contract that called this callback
        }

        // refund tokens not used (ft_resolve_transfer)
        U128(0)
    }
}
...
impl FungibleTokenCore for Contract {
    ...
    fn ft_transfer_call(
        ...
        assert_attached_deposit();
        let sender_id = env::predecessor_account_id();
        let amount = amount.into();
        self.internal_transfer(&sender_id, receiver_id.as_ref(), amount, memo);
        // Initiating receiver's call and the callback
        ext_fungible_token_receiver::ft_on_transfer(
            ValidAccountId::try_from(sender_id.clone()).unwrap(),
            amount.into(),
            msg,
    ...
    fn ft_resolve_transfer(
        ...
        let unused_amount = match env::promise_result(0) {
            PromiseResult::NotReady => {
                env::log("NotReady".as_bytes());
                unreachable!()
            },
            PromiseResult::Successful(value) => {
                env::log(format!("Successful {:?}", value).as_bytes());
            ...
            PromiseResult::Failed => {
                env::log(format!("Failed {:?}", env::promise_result(0)).as_bytes());
